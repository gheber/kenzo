...
(DEFUN CYLN-P (object)
   ...
   (the boolean       
     (and (consp object)            
          (typep (car object) '(member :CYLNA1 :CYLNB :CYLNA2))
          (typep (cdr object) 'gnrt))))

(DEFTYPE CYLN () '(satisfies CYLN-P))  
       ...
       (with-slots ((cmprA cmpr)) Acc
         (with-slots ((cmprb cmpr)) Bcc
           (let ((cylinder (cylinder f)))
            ...
            (build-rdct :f lf :g lg :h lh :orgn `(cylinder left reduction ,f ,g ,h ,k)))))))))
 ...
 (DEFMETHOD SEARCH-EFHM (smst (orgn (eql 'k-g-1)))
   ...
   (the homotopy-equivalence
    (let* ((group (second (orgn smst)))
       ...
       (build-hmeq :lrdct lrdct :rrdct rrdct :orgn  `(cylinder-hmeq ,f ,g ,h ,k )))))
