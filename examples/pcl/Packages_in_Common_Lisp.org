...
* Internal symbols.
  ...
  *package*
  ...
  symbols and assign it to the symbol =list=, and then examine the internal
  ...
  ="COMMON-LISP"=, is "locked". It is possible to unlock such a symbol, which
  is a little technical, but using =(symbol-value 'list)= as above gets the job
  done.
