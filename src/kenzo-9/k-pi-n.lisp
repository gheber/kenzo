...
(DEFUN K-Z-1-GRML (dmns crpr)
   ...
   (the absm
	 ...
	 (do ((indx 1 (1+ indx))
	     ...
	     (declare
	        (type fixnum indx dgop1 dgop2 bark rslt-dgop)
		(list rslt-bar))
	    ...
	    (let ((del-1 (first hat))
		 ...
		 (let ((bar01 (if (oddp dgop2)
		       ...
		       (bar02 (if (oddp dgop1)
				 0
				 (first bar1))))
		   (declare (type fixnum bar01 bar02))
		   (let ((bar12 (- bar02 bar01)))
		     (declare (type fixnum bar02))
		     (if (zerop bar01)
			(1dgnr 0 del-1)
		        (if (zerop bar12)
			   (1gnr 1 del-1)
...
(DEFUN CIRCLE-APRD-IMPL (degr tnpr)
  (declare (type fixnum degr) (type tnpr) (ignore degr))
  (the cmbn
...
(DEFUN KZ1-RDCT-H-INTR (dmns bar)
  ...
  (the cmbn
    (progn
      ...
      (let ((k1 (first bar))
            (tail (rest bar)))
        (declare (type fixnum k1) (list tail))
        (if (plusp k1)
            (do ((rslt +empty-list+
                       (cons (term -1
                                   (cons 1 (cons i tail)))
...
(DEFUN Z-FUNDAMENTAL-GMSM (dmns pi-elm)   ;;; pi-elm not equal to 0
   (declare (type fixnum dmns pi-elm))
   (the gmsm
      (if (= 1 dmns)
         (list pi-elm)
...
(DEFUN INTERESTING-FACES (face small-dmns high-dmns gmsm)
  (declare
    (type face face)
    (type fixnum small-dmns high-dmns)
    (type gmsm gmsm))
  (the list
