...
(DEFUN SHUFFLE-SIGN (dgop)
   ...
   (the fixnum
      (do ((rslt +1)
         ...
         (declare (type fixnum rslt bit-position factor))
         ...
         (let* ((limits (make-array (+ degr 2)))
            ...
            (dotimes (dgop1 (1+ mask))
               ...
               (let* ((i (- degr (logcount dgop1)))
                  ...
                  (declare (type fixnum i dgop2 sign))
                         ...
                         (let ((absm1 (absm1 crpr))
                            ...
                            (do ((n-alpha n (1- n-alpha))
                               ...
                               (do ((beta 0 (1+ beta))
                                   ...
                                   ((= beta n-alpha))
                                  (declare
                                     (type fixnum beta i sign dim-right-d)
                                     (type absm right-d))
            ...
            (let ((cmbn-i-tail cmbn-i))
               ...
               (do ((dgop1 0 (1+ dgop1)))
                  ...
                  (let ((p (- n+1 (logcount dgop1)))
                        (dgop2 (- mask dgop1)))
                     (declare (type fixnum p dgop2))
                     (unless (= p i)
                        (let ((q (- (+ n+1 i) p))
                              (sign (shuffle-sign dgop2))
                              (dgop1-ashed (ash dgop1 i))
                              (dgop2-ashed (ash dgop2 i)))
                           (declare (type fixnum q sign dgop1-ashed dgop2-ashed))
...
(DEFUN PHI (smst1 smst2)
  (declare (type simplicial-set smst1 smst2))
  (the morphism
    (chcm-vf-reduction-h (crts-prdc smst1 smst2) #'ez-vf)))
