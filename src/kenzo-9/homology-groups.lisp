...
(defun creer-matrice (n1 n2)
  ;; null matrix with n1 rows and n2 colums
...
(DEFMETHOD PRINT-OBJECT ((mtrx matrice) stream)
  ...
  (the matrice (progn
    (let ((nlig (nlig mtrx))
	  (ncol (ncol mtrx)))
      (declare (type fixnum nlig ncol))
      (format stream "~%========== MATRIX ~D row(s) + ~D column(s) ====="
	      nlig ncol)
      (do ((ilig 1 (1+ ilig)))
	  ((> ilig nlig))
	  (declare (type fixnum ilig))
	(format stream "~%R~D=" ilig)
...
#|
(setf m (random-matrix 3 6 0.5 10))
(do ((i 1 (1+ i)))
    ((> i 3))
  (do ((j 1 (1+ j)))
      ((> j 6))
    (print (list i j (terme m i j)))))
|#
...
(defun matrix-line (m i)
  (declare (type matrice m) (type fixnum i))
  (the list
    (let ((start (baselig m i)))
      (do ((p (left start) (left p))
           (rslt +empty-list+ (cons (list (icol p) (val p)) rslt)))
          ((eq p start) rslt)))))

#|
(setf m (creer-matrice 3 6))
(maj-ligne m 1 '((1 2) (3 4) (5 6)))
m
(maj-ligne m 2 '((1 2) (3 4) (5 6)))
m
(dotimes (i 3)
  (print (matrix-line m (1+ i))))
|#
  
(defun matrix-transpose (m)
  (declare (type matrice m))
  (the matrice
    (let ((rslt (creer-matrice (ncol m) (nlig m))))
      (do ((ilig 1 (1+ ilig)))
          ((> ilig (nlig m)))
        (maj-colonne rslt ilig (matrix-line m ilig)))
      rslt)))

#|
(setf m (mat-aleat 5 8 0.5 10))
(matrix-transpose m)
|#
