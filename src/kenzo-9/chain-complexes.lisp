...
(DEFPARAMETER +MAXIMAL-DIMENSION+ 22)

(DEFVAR *MRPH-LIST*)
(SETF *MRPH-LIST* +empty-list+)
(PUSHNEW '*MRPH-LIST* *list-list*)

(DEFGENERIC ?2 (arg1 arg2))
...
(DEFUN BUILD-MRPH (&key sorc trgt degr intr strt orgn)
   (declare
      ...
      ;; Sometimes the trgt is temporarily nil.
      (type (or null chain-complex) trgt)
      (type fixnum degr)
         ...
         (let ((mrph (make-instance 'morphism
           ...
           (setf (slot-value mrph 'rslts)
             (ecase strt
               ...
               (:cmbn nil)))
           (unless trgt
             (slot-makunbound mrph 'trgt))
           (push mrph *mrph-list*)
           mrph))))

;;; FUNCTIONS

;; The *start-stack* variable maintains a stack used to compute
;; the time *locally* used to compute some image

(DEFVAR *START-STACK* +empty-list+)

(DEFPARAMETER +TOO-MUCH-TIME+ -1)

(DEFVAR *RESULTS-MAX* (expt 10 7))

(DEFVAR *RESULTS-COEF* 0.5)
...
(DEFUN MRPH-GNRT (cmpr2 intr degr gnrt strt memory &optional (left -1) (right (fill-pointer memory)))
  (declare
   (type intr-mrph intr)
   ;; (function (type fixnum gnrt) cmbn)
   (type cmprf cmpr2)
   (type fixnum degr left right)
   (type gnrt gnrt)
   (vector memory)
   (special *results-n*)
   (type fixnum *results-n*))
        ...
        (let ((value (funcall intr degr gnrt))
          ...
          (if (> runtime +too-much-time+)
              ...
              (let ((old-length (vector-push-extend nil memory)))
                ...
                (when (eq strt :rcrs)
                  (do ((new-right right (1+ new-right)))
                      ((null (aref memory new-right))
                       (setf right new-right))
                    (declare (type fixnum new-right))
                    (when (eq :less (funcall cmpr2 gnrt (result-gnrt (aref memory new-right))))
                      (setf right new-right)
                      (return))))
      ...
      (let ((n-cmbn-list +empty-list+))
        ...
        (do ((mark list (cdr mark))
          ...
          (multiple-value-bind (rslt new-left)
              (mrph-gnrt scmpr2 intr degr (-gnrt mark) strt memory left)
            (setf left new-left)
                     ...
                     (ecase strt
                        ((:gnrt :rcrs)
                         (mrph-gnrt scmpr intr
                            degr gnrt strt (svref (rslts mrph) degr)))
                        (:cmbn
                     ...
                     (prog1
                         (if (cmbn-zero-p cmbn)
                           ...
                           (mrph-cmbn scmpr tcmpr intr
                                      cmbn strt (svref (rslts mrph) (cmbn-degr cmbn))))
                       ;;                     (profiler-off mrph)
                       )))))))))
