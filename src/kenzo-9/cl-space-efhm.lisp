...
(DEFUN CLASSIFYING-SPACE-EFHM-2 (smgr &aux (efhm (efhm smgr)))
  (declare (type simplicial-group smgr)
           (type (or reduction equivalence) efhm))
  (setf efhm
    (etypecase efhm
      (equivalence efhm)
      (reduction (build-hmeq :lrdct (trivial-rdct (tcc efhm))
                             :rrdct efhm
                             :orgn `(classifying-space-efhm-2 ,smgr)))))
  (let ((left-hmeq (cs-left-hmeq smgr))l
        (right-hmeq (bar efhm)))
    (declare (type equivalence left-hmeq right-hmeq))
