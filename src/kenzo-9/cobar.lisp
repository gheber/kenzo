...
(DEFINE-CONSTANT +NULL-ALLP+ (make-allp :list +empty-list+))
   ...
   (the basis
      (progn
         ...
         (labels ((rslt (degr length)
                     (declare (type fixnum degr length))
                     (when (= 1 length)
                        (return-from rslt
                           (mapcar
                              #'(lambda (item)
...
(DEFMETHOD VRTC-COBAR ((hmeq equivalence))
   (the equivalence
