...
(DEFINE-CONSTANT +NULL-LOOP+ (make-loop :list +empty-list+))
...
#|
(cat-init)
(setf p (r-proj-space :infinity 3))
...
#|  ;; in EAT.
  ...
  (DEFUN PR-INFINITY-GDL (k)
     ...
     (flet ((rslt (indx dmns gsm)
             (declare (type fixnum indx dmns gsm))
             (if (<= dmns k)
