...
(DEFUN Z-WHITEHEAD-SINTR (smst n chml-clss
  ...
  (declare
   ...
   (type gmsm idnt))
  (flet ((rslt (dmns gmsm)
           (declare (type fixnum dmns) (type gmsm gmsm))
           (if (< dmns n)
...
(equalp (funcall tw 3 (z-fundamental-gmsm 3 55))
...
(DEFUN Z-WHITEHEAD (smst chml-clss &aux (n (- (degr chml-clss))))
  ...
  (the fibration
       (build-smmr
        ...
        :orgn `(z-whitehead ,smst ,chml-clss))))
...
(equalp (funcall tw 3 (Z2-fundamental-gmsm 3 1))
...
(DEFUN Z2-WHITEHEAD (smst chml-clss &aux (n (- (degr chml-clss))))
  ...
  (the fibration
       (build-smmr
        ...
        :orgn `(Z2-whitehead ,smst, chml-clss))))

#|
(cat-init)
(setf m (moore 2 4))
(setf chml-clss (chml-clss m 4))
(setf mf (z2-whitehead m chml-clss))
