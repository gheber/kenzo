...
(DECLAIM (FTYPE function build-mrph))
...
(DEFUN BUILD-MRPH (&key sorc trgt degr intr strt orgn)
  (declare
   ...
   (fixnum degr)
   ;; BUILD-MRPH  can be  called  by BUILD-SMMR  with intr  undefined.
   ;; BUILD-SMMR deduces later the intr slot from its sintr slot.
   (type (or null intr-mrph) intr)
   (type strt strt)
   (list orgn))
  (the morphism
    (progn
