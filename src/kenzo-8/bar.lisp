...
#|
(cat-init)
(setf v (vrtc-bar (delta-infinity)))
(defun random-abar (length)
  (let ((rslt nil))
    (dotimes (i length)
      (let* ((gmsm (random (mask 6)))
             (dmns (1- (logcount gmsm))))
        (when (plusp dmns)
          (push (brgn (1+ dmns) gmsm) rslt))))
    (make-abar :list rslt)))
...
(DEFUN BAR-INTR-HRZN-DFFR (aprd)
  ...
  (labels ((rslt
            (degr iabar)
            (declare (fixnum degr) (type iabar iabar))
            (the icmbn
              (progn
                (unless (cdr iabar)
                  (return-from rslt +empty-list+))
...
(defun aleat-tc ()
  (do ((tdegr 0 (+ tdegr degr))
      ...
      ((> tdegr 8) (setf *tc* (cmbn tdegr 1 (make-abar :list rslt))))))
(aleat-tc)
(defun aleat-bc ()
  (do ((tdegr 0 (+ tdegr degr))
       (degr (+ 2 (random 3)) (+ 2 (random 3)))
       ...
       (rslt nil (cons (brgn degr gnrt) rslt)))
      ((> tdegr 8) (setf *bc* (cmbn tdegr 1 (make-abar :list rslt))))))
(aleat-bc)
(defun c ()
  (aleat-tc)
  (aleat-bc)
