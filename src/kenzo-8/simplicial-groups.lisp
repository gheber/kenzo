...
(DECLAIM (FTYPE function smgr-kfll))


(DEFMETHOD SLOT-UNBOUND (class (smgr simplicial-group)
                               (name (eql 'kfll)))
  (declare (ignore class))
  ;; Bug in  the type calculation  in SBCL.   Sbcl obtains nil  as the
  ;;   unique possible  result and claims not being  able to  obtain a
  ;;   morphism.
  (the (or null kfll) ;; should be (the kfll ...
    (progn
      (push smgr *kan-list*)
...
(declaim (ftype function smgr-aprd))
...
(DEFMETHOD SLOT-UNBOUND (class (smgr simplicial-group)
  ...
  ;;   unique possible  result and claims not being  able to  obtain a
  ;;   morphism.
  (the (or null morphism) ;; should be (the morphism ...
    (progn
      (push smgr *hopf-list*)
