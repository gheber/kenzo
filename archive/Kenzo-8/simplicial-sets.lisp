...
(DEFMETHOD PRINT-OBJECT ((absm absm) stream)
...
(DECLAIM (FTYPE function a-face4))
...
(DECLAIM (FTYPE function face*-bndr face-bndr intr-diagonal))
...
(DEFUN A-BASIS2 (basis dmns)
  (declare
   ...
   (fixnum dmns))
  (the list
    (let ((basis-array (make-array (1+ dmns)
                                   ;; strange sbcl behaviour:
                                   ;; "default" initial-element = 0
                                   #+SBCL :initial-element
                                   #+SBCL +empty-list+
                                   :element-type 'list)))
