...
(DEFUN SMGR-FIBRATION (smgr)
  ...
  (the fibration
    (simplicial-mrph
      ...
      :dfnt `(simplicial-group-fibration ,smgr))))
...
(DEFUN SMGR-CRTS-CONTRACTION (smgr)
  ...
  (the morphism
    (morphism
      ...
      :degr +1
      :intr (smgr-crts-contraction-intr (bsgn smgr))
      :strt :gnrt
      :dfnt `(smgr-crts-contraction ,smgr))))

#|
  (cat-init)
  (setf chi (smgr-crts-contraction (k-z-1)))
  (setf rdct (reduction :f (zero-mrph (sorc chi) (z-chcm))
