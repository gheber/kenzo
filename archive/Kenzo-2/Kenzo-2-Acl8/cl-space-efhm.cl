...
(DEFUN CS-HAT-RIGHT-PERTURBATION (smgr
  ...
  (the morphism
	...
	:strt :gnrt
	:dfnt `(cs-hat-right-perturbation ,smgr))))
...
(DEFUN CS-PRE-LEFT-EQVL-LEFT-REDUCTION-INTR-F (cmbn)
...
#|
  (cs-pre-left-eqvl-left-reduction-intr-f
...
(DEFUN CS-PRE-LEFT-EQVL-LEFT-REDUCTION-F (smgr
  ...
  (the morphism
    (morphism
      ...
      :degr 0
      :intr #'cs-pre-left-eqvl-left-reduction-intr-f
      :strt :cmbn
      :dfnt `(cs-pre-left-eqvl-left-reduction-f ,smgr))))

(DEFUN CS-PRE-LEFT-EQVL-LEFT-REDUCTION-INTR-G (idnt)
...
#|
  (setf g (cs-pre-left-eqvl-left-reduction-intr-g '()))
  (funcall g (cmbn 3 14 (gbar 3 0 'a 1 'b 0 '())))
|#

(DEFUN CS-PRE-LEFT-EQVL-LEFT-REDUCTION-G (smgr
					   &aux (hat-u-t (cs-hat-u-t smgr))
					        (classifying-space (classifying-space smgr)))
  (declare
    (type simplicial-group smgr)
  ...
  (the morphism
    (morphism
      ...
      :trgt hat-u-t
      :degr 0
      :intr (cs-pre-left-eqvl-left-reduction-intr-g (bsgn smgr))
      :strt :cmbn
      :dfnt `(cs-pre-left-eqvl-left-reduction-g ,smgr))))

(DEFUN CS-PRE-LEFT-EQVL-LEFT-REDUCTION-INTR-H (cmpr idnt)
  (declare (type gmsm idnt))
  (flet ((rslt (cmbn)
	   (declare (type cmbn cmbn))
	   (the cmbn
...
(DEFUN CS-PRE-LEFT-EQVL-LEFT-REDUCTION-H (smgr
					   &aux (hat-u-t (cs-hat-u-t smgr)))
  (declare
    (type simplicial-group smgr)
    (type chain-complex hat-u-t))
  (the morphism
    (morphism
      ...
      :trgt hat-u-t
      :degr +1
      :intr (cs-pre-left-eqvl-left-reduction-intr-h (cmpr hat-u-t) (bsgn smgr))
      :strt :cmbn
      :dfnt `(cs-pre-left-eqvl-left-reduction-h ,smgr))))



(DEFUN CS-PRE-LEFT-EQVL-LEFT-REDUCTION (smgr)
  (declare (type simplicial-group smgr))
  (the reduction
    (reduction
      :f (cs-pre-left-eqvl-left-reduction-f smgr)
      :g (cs-pre-left-eqvl-left-reduction-g smgr)
      :h (cs-pre-left-eqvl-left-reduction-h smgr)
      :dfnt `(cs-pre-left-eqvl-left-reduction ,smgr))))
    ...
    (do ((rslt nil)
       ...
       (push (brgn (1+ degr)
                   (let ((list (make-list degr)))
                     ...
                     list))
             rslt)))
  (setf rdct (cs-pre-left-eqvl-left-reduction (k-z-1)))
  (pre-check-rdct rdct)
...
(DEFUN CS-LEFT-EQVL-LEFT-REDUCTION (smgr
				     &aux (pre-left-eqvl-left-reduction
					     (cs-pre-left-eqvl-left-reduction smgr))
				          (hat-left-perturbation
					     (cs-hat-left-perturbation smgr)))
  (declare
    (type simplicial-group smgr)
    (type reduction pre-left-eqvl-left-reduction)
    (type morphism hat-left-perturbation))
  (the reduction
    (progn
      (dstr-change-sorc-trgt hat-left-perturbation
	:new-sorc (tcc pre-left-eqvl-left-reduction)
	:new-trgt (tcc pre-left-eqvl-left-reduction))
      (let ((rslt (special-bpl-2 pre-left-eqvl-left-reduction
				 hat-left-perturbation)))
	(declare (type reduction rslt))
	(with-slots (tcc f g h) rslt
	  (setf tcc (cs-left-eqvl-hat smgr)
    ...
    (do ((rslt nil)
       ...
       (push (brgn (1+ degr)
                   (let ((list (make-list degr)))
                     ...
                     list))
             rslt)))
  (setf rdct (cs-left-eqvl-left-reduction (k-z-1)))
  (pre-check-rdct rdct)
  (setf *tc* (cmbn 0 1 (tnpr 0 (tnpr 0 +null-gbar+ 0 '()) 0 +null-abar+))
        *bc* (cmbn 0 1 +null-gbar+))
  (check-rdct)
...
(DEFUN CS-PRE-LEFT-EQVL-RIGHT-REDUCTION-INTR-F (cmbn)
  (declare (type cmbn cmbn))
  (the cmbn
     (with-cmbn (degr list) cmbn
     (let ((rslt +empty-list+))
...
#|
  (cs-pre-left-eqvl-right-reduction-intr-f
...
(DEFUN CS-PRE-LEFT-EQVL-RIGHT-REDUCTION-F (smgr
					     &aux (hat-t-u (cs-hat-t-u smgr))
					          (bar (bar smgr)))
  (declare
    (type simplicial-group smgr)
  ...
  (the morphism
    (morphism
      ...
      :intr #'cs-pre-left-eqvl-right-reduction-intr-f
      :strt :cmbn
      :dfnt `(cs-pre-left-eqvl-right-reduction-f ,smgr))))

(DEFUN CS-PRE-LEFT-EQVL-RIGHT-REDUCTION-INTR-G (idnt)
  (declare (type gmsm idnt))
  (flet ((rslt (cmbn)
	   (declare (type cmbn cmbn))
	   (the cmbn
...
#|
  (setf g (cs-pre-left-eqvl-right-reduction-intr-g 'i))
  (funcall g (cmbn 3 2 (abar 1 'a 2 'b)))
|#

(DEFUN CS-PRE-LEFT-EQVL-RIGHT-REDUCTION-G (smgr
					    &aux (hat-t-u (cs-hat-t-u smgr))
					         (bar (bar smgr))
						 (idnt (bsgn smgr)))
  (declare
  ...
  (the morphism
    (morphism
      ...
      :intr (cs-pre-left-eqvl-right-reduction-intr-g idnt)
      :strt :cmbn
      :dfnt `(cs-pre-left-eqvl-right-reduction-g ,smgr))))

(DEFUN CS-PRE-LEFT-EQVL-RIGHT-REDUCTION-H (smgr
					    &aux (bar (bar smgr))
					         (tnpr-contraction
...
(DEFUN CS-PRE-LEFT-EQVL-RIGHT-REDUCTION (smgr)
  (declare (type simplicial-group smgr))
  (the reduction
    (reduction
      :f (cs-pre-left-eqvl-right-reduction-f smgr)
      :g (cs-pre-left-eqvl-right-reduction-g smgr)
      :h (cs-pre-left-eqvl-right-reduction-h smgr)
      :dfnt `(cs-pre-left-eqvl-right-reduction ,smgr))))
    ...
    (do ((rslt nil)
       ...
       (push (brgn (1+ degr)
                   (let ((list (make-list degr)))
                     ...
                     list))
             rslt)))
  (setf rdct (cs-pre-left-eqvl-right-reduction (k-z-1)))
  (pre-check-rdct rdct)
  (setf *tc* (cmbn 0 1 (tnpr 0 (tnpr 0 +null-gbar+ 0 '()) 0 +null-abar+))
...
(DEFUN CS-LEFT-EQVL-RIGHT-REDUCTION (smgr
				      &aux (pre-reduction
					     (cs-pre-left-eqvl-right-reduction smgr))
				           (perturbation
					     (cs-hat-right-perturbation smgr)))
  (declare
    (type simplicial-group smgr)
    ...
    (do ((rslt nil)
       ...
       (push (brgn (1+ degr)
                   (let ((list (make-list degr)))
                     ...
                     list))
             rslt)))
  (setf rdct (cs-left-eqvl-right-reduction (k-z-1)))
  (pre-check-rdct rdct)
  (setf *tc* (cmbn 0 1 (tnpr 0 (tnpr 0 +null-gbar+ 0 '()) 0 +null-abar+))
        *bc* (cmbn 0 1 +null-abar+))
  (check-rdct)
...
(DEFUN CS-LEFT-EQVL (smgr)
  (declare (type simplicial-group smgr))
  (the equivalence
    (equivalence
     :lrdct (cs-left-eqvl-left-reduction smgr)
     :rrdct (cs-left-eqvl-right-reduction smgr)
     :dfnt `(cs-left-eqvl ,smgr))))

#|
  (cat-init)
  (setf h (cs-left-eqvl (k-z-1)))
...
(DEFUN CLASSIFYING-SPACE-EFHM (smgr)
  (declare (type simplicial-group smgr))
  (let ((left-eqvl (cs-left-eqvl smgr))
	(right-eqvl (bar (efhm smgr))))
    (declare (type equivalence left-eqvl right-eqvl))
    (cmps left-eqvl right-eqvl)))

(DEFMETHOD SEARCH-EFHM (classifying-space (dfnt (eql 'classifying-space)))
  (declare (type simplicial-set classifying-space))
  (classifying-space-efhm (second (dfnt classifying-space))))
