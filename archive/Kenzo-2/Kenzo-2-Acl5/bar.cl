...
(DEFMETHOD VRTC-BAR ((chcm chain-complex))
   (the chain-complex
     (let ((dfnt `(vrtc-bar ,chcm)))
       (declare (type list dfnt))
       (already vrtc-bar dfnt)
       (with-slots (cmpr basis dffr) chcm
         ...
         (chain-complex
          ...
          :bsgn +null-abar+
          :dffr-intr (bar-intr-vrtc-dffr dffr)
          :dffr-strt :gnrt
          :dfnt dfnt)))))
  ...
  (the chain-complex
    (let ((vrtc-bar (vrtc-bar algebra))
      ...
      (let ((rslt (chain-complex
                   ...
                   :bsgn +null-abar+
                   :dffr-intr (bar-intr-dffr (dffr vrtc-bar) bar-hrzn-dffr)
                   :dffr-strt :gnrt
                   :dfnt `(add ,vrtc-bar ,bar-hrzn-dffr))))
   ...
   (the morphism
      (if (eq (first (dfnt h)) 'zero-mrph)
         ...
         (morphism
            ...
            :strt :gnrt
            :dfnt `(hmtp-vrtc-bar ,h ,gf)))))

(DEFMETHOD VRTC-BAR ((rdct reduction))
  (the reduction
    (let ((dfnt `(vrtc-bar ,rdct)))
      (declare (type list dfnt))
...
(DEFMETHOD VRTC-BAR ((eqvl equivalence))
  (the equivalence
    (let ((dfnt `(vrtc-bar ,eqvl)))
      (declare (type list dfnt))
      (already vrtc-bar dfnt)
      (if (eq (first (dfnt eqvl)) 'trivial-eqvl)
          (trivial-eqvl (vrtc-bar (lbcc eqvl)))
...
(DEFMETHOD BAR ((eqvl equivalence))
  (unless (typep (lbcc eqvl) 'algebra)
    (error "In (BAR eqvl), the LBCC should be a algebra."))
  (the equivalence
    (if (eq (first (dfnt eqvl)) 'trivial-eqvl)
        (trivial-eqvl (bar (lbcc eqvl)))
      (add (vrtc-bar eqvl) (bar-hrzn-dffr (lbcc eqvl))))))
