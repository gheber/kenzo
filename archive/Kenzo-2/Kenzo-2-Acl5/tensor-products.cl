...
(DEFUN TNSR-PRDC-DFFR-INTR (dffr1 dffr2)
...
#|
  (cat-init)
  (setf chcm (chain-complex :cmpr #'s-cmpr :dffr-strt :gnrt :dfnt '(test-1)))
  (setf dffr (morphism :sorc chcm :trgt chcm :degr -1
                         :intr #'(lambda (degr gnrt)
                                    (ecase gnrt
                                       (a (cmbn (1- degr) 2 'd1a -3 'd2a))
                                       (b (cmbn (1- degr) 3 'd1b -4 'd2b))))
                         :strt :gnrt :dfnt '(test-2)))
  (setf rslt (tnsr-prdc-dffr-intr dffr dffr))
      ...
      (with-slots ((cmpr2 cmpr) (basis2 basis) (dffr2 dffr)) chcm2
        ...
        (let ((rslt
               (chain-complex
                ...
                :dffr-intr (tnsr-prdc-dffr-intr dffr1 dffr2)
                :dffr-strt :gnrt
                :dfnt `(tnsr-prdc ,chcm1 ,chcm2))))
...
#|
  (cat-init)
  (setf chcm (chain-complex :cmpr #'s-cmpr :dffr-strt :gnrt :dfnt '(test-1)))
  (setf mrph (morphism :sorc chcm :trgt chcm :degr 0
                         :intr #'(lambda (degr gnrt)
                                    (ecase gnrt
  ...
  (funcall rslt 5 (tnpr 3 'a 2 'b))
  (setf mrph (morphism :sorc chcm :trgt chcm :degr 1
                         :intr #'(lambda (degr gnrt)
                                    (ecase gnrt
                                       (a (cmbn (1+ degr) 2 'd1a -3 'd2a))
                                       (b (cmbn (1+ degr) 3 'd1b -4 'd2b))))
                         :strt :gnrt :dfnt '(test-3)))
  (setf rslt (tnsr-prdc-intr mrph mrph))
  (funcall rslt 6 (tnpr 2 'a 4 'b))
  (funcall rslt 5 (tnpr 3 'a 2 'b)))
|#
...
(DEFMETHOD TNSR-PRDC ((mrph1 morphism) (mrph2 morphism))
  (the morphism
   ...
   (with-slots ((sorc2 sorc) (trgt2 trgt) (degr2 degr)) mrph2
      (the morphism
	 (progn
	   (when (and (eq 'idnt-mrph (first (dfnt mrph1)))
		      (eq 'idnt-mrph (first (dfnt mrph2))))
	      (return-from tnsr-prdc
		 (idnt-mrph (tnsr-prdc sorc1 sorc2))))
	   (when (or (eq 'zero-mrph (first (dfnt mrph1)))
		     (eq 'zero-mrph (first (dfnt mrph2))))
	      (return-from tnsr-prdc
...
(DEFMETHOD TNSR-PRDC ((rdct1 reduction) (rdct2 reduction))
  (the reduction
     (progn
       (when (and (eq 'trivial-rdct (first (dfnt rdct1)))
		  (eq 'trivial-rdct (first (dfnt rdct2))))
          (return-from tnsr-prdc
...
(DEFMETHOD TNSR-PRDC ((eqvl1 equivalence)
		      (eqvl2 equivalence))
  (the equivalence
     (equivalence
      :lrdct (tnsr-prdc (lrdct eqvl1) (lrdct eqvl2))
      :rrdct (tnsr-prdc (rrdct eqvl1) (rrdct eqvl2))
      :dfnt `(tnsr-prdc ,eqvl1 ,eqvl2))))

(DEFMETHOD SEARCH-EFHM (chcm (dfnt (eql 'tnsr-prdc)))
  (declare (type chain-complex chcm))
  (the equivalence
       (tnsr-prdc (efhm (second (dfnt chcm)))
		  (efhm (third (dfnt chcm))))))
