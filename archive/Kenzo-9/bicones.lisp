...
(DEFUN BICN-PRINT (bicn stream depth)
  (declare
   (type bicn bicn) (type stream stream)
   (ignore depth))
  (the bicn
    (with-bicn (bcnx ibicn) bicn
               ;; Correction 1-1-2
...
(DEFMETHOD CMPS ((hmeq1 equivalence) (hmeq2 equivalence) &optional dummy)
  ;;; hmeq1 on the lefthand side
  ;;; hmeq2 on the right hand side
  ;;; rbcc(hmeq1) = lbcc(hmeq2) required
   (declare (ignore dummy))
   (the equivalence
      (progn
         (when (eq 'trivial-hmeq (orgn hmeq2))
            (return-from cmps hmeq1))
         (when (eq 'trivial-hmeq (orgn hmeq1))
            (return-from cmps hmeq2))
