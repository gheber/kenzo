...
(DEFUN ABSM-LOOPABSM (cmpr dmns absm loop-absm)
   (declare
      (type cmprf cmpr)
      (type fixnum dmns)  ;; dimension of ABSM
                  ...
                  (with-apowr (dgop22 gmsm2 expn2) apowr
                     (if (and (= dgop1 (dgop*dgop dgop2 dgop22))
                        ...
                        (multiple-value-bind (dgop dgop1 dgop2) (extract-common-dgop dgop1 dgop2)
                           (declare (type fixnum dgop dgop1 dgop2))
                           (absm dgop
                              (make-loop
...
(DEFUN POP-FIRST-ABSM (dmns iloop)
   ...
   (the (values (member -1 0 1) (or null absm) absm)
      (progn
         (unless iloop
 ...
 (labels (
  ...
  (h-hat (g-n)
         ...
         (multiple-value-bind (expn y-n+1 gp-n) (pop-first-absm n g-n)
           (declare
            (type fixnum expn)
            (type (or null absm) y-n+1)
            (type (or null absm) gp-n))        
           (ecase expn
             (0 (zero-cmbn (1+ n)))
             (+1
