...
(when (or (string= (package-name (find-package 'cat)) "CAT-7")
    ...
    (test random-matrix
          (cat-9:random-matrix 2 3 10))

    (test copy-mtrx
          (let ((m (cat-9:random-matrix 3 4 10)))
            (is (equalp m (cat-9:copy-mtrx m)))))

    (test left-submatrix
          (let ((m (cat-9:random-matrix 3 4 10)))
            (cat-9:left-submatrix m 2)))

    (test mtrx-prdc
          (let ((m1 (cat-9:random-matrix 2 3 10))
                (m2 (cat-9:random-matrix 3 2 10)))
    ...
    (test chcm-mtrx
      ...
      (let ((d (cat-9:delta 5))
        ...
        (dotimes (i 6)
          (print (array-dimensions (cat-9:chcm-mtrx m i))))))

    (test line-op
          (let ((m (cat-9:random-matrix 3 4 10)))
            (cat-9:line-op m 1 3 2 0)))
    ...
    (test column-op-5
          (let* ((p (cat-9:idnt-mtrx 4))
                 ...
                 (list (list p p-1 m q q-1))
                 (t1 (cat-9:mtrx-prdc p (cat-9:mtrx-prdc m q-1))))
            (cat-9:column-op-5 list 0 3 1 3)
            (is (equalp t1 (cat-9:mtrx-prdc p (cat-9:mtrx-prdc m q-1))))
            (cat-9:mtrx-prdc p p-1)
    ...
    (test column-swap-5
          (let* ((p (cat-9:idnt-mtrx 4))
                 ...
                 (list (list p p-1 m q q-1))
                 (t1 (cat-9:mtrx-prdc p (cat-9:mtrx-prdc m q-1))))
            (cat-9:column-swap-5 list 0 1 3)
            (is (equalp t1 (cat-9:mtrx-prdc p (cat-9:mtrx-prdc m q-1))))
            (cat-9:mtrx-prdc p p-1)
            (cat-9:mtrx-prdc q q-1)))
    ...
    (test column-minus
          (let ((m (cat-9:random-matrix 3 4 10)))
            (cat-9:line-minus m 1 2)
            (cat-9:column-minus m 1 2)))
    ...
    (test column-minus-5
          (let* ((p (cat-9:idnt-mtrx 4))
                 ...
                 (list (list p p-1 m q q-1))
                 (t1 (cat-9:mtrx-prdc p (cat-9:mtrx-prdc m q-1))))
            (cat-9:line-minus-5 list 0 3)
            (is (equalp t1 (cat-9:mtrx-prdc p (cat-9:mtrx-prdc m q-1))))
            (cat-9:mtrx-prdc p p-1)
            (cat-9:mtrx-prdc q q-1)
            (cat-9:column-minus-5 list 0 2)
            (is (equalp t1 (cat-9:mtrx-prdc p (cat-9:mtrx-prdc m q-1))))
            (cat-9:mtrx-prdc p p-1)
            (cat-9:mtrx-prdc q q-1)))

    (test minimal-term
      (let ((m (cat-9:random-matrix 4 5 10)))
    ...
    (test minimal-rest-1
    (let ((m (cat-9:random-matrix 4 5 10)))
    (cat-9:minimal-rest-1 m 1)))

    (test minimal-rest-2
    (let ((m (cat-9:random-matrix 4 5 10)))
    (cat-9:minimal-rest-2 m 1)))
    |#
