...
We\index{$K(\pi,n)$} have already described $K(\mathbb{Z},1)$ and $K(\mathbb{Z}_2,1)$ in the chapter on simplicial groups.
...
<AbSm - <<GBar<- (1 2)><0 NIL><- NIL>>>>
\end{verbatim}}
As $K(\mathbb{Z}, 1)$ and $K({\Z_2}, 1)$ are Abelian simplicial groups, we may iterate the classifying
